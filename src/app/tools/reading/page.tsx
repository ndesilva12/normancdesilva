...
export default function ReadingPage() {
  const [raindropItems, setRaindropItems] = useState<RaindropItem[]>([]);
  ...
  const [selectedSource, setSelectedSource] = useState<"raindrop" | "rss">("raindrop");
  ...
  const checkConnectionStatus = async () => {
    try {
      ...
      // Always fetch RSS items as they don't require auth
      fetchRssItems();
    } catch (err) {
  ...
  const fetchRaindropItems = async (collectionId?: number) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const handleCollectionClick = (collectionId: number) => {
    ...
    fetchRaindropItems(collectionId);
  };
  ...
  const handleSourceChange = (source: "raindrop" | "rss") => {
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", paddingTop: "64px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "24px", paddingTop: "24px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: isMobile ? "wrap" : "nowrap" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", backgroundColor: "rgba(255, 255, 255, 0.05)", border: "1px solid var(--glass-border)", borderRadius: "12px", padding: "0 8px", height: "48px" }}>
                <button
                  onClick={() => handleSourceChange("raindrop")}
                  style={{
                    display: "flex",
                    alignItems: "center",
                    gap: "8px",
                    ...
                    height: "40px",
                    borderRadius: "8px",
                    backgroundColor: selectedSource === "raindrop" ? "rgba(100, 255, 100, 0.1)" : "transparent",
                    border: selectedSource === "raindrop" ? "1px solid rgba(100, 255, 100, 0.2)" : "none",
                    color: selectedSource === "raindrop" ? "var(--accent)" : "var(--foreground-muted)",
                    fontSize: "14px",
                    cursor: "pointer",
                    transition: "all 0.2s",
                  }}
                ...
                <button
                  onClick={() => handleSourceChange("rss")}
                  style={{
                    display: "flex",
                    alignItems: "center",
                    gap: "8px",
                    ...
                    height: "40px",
                    borderRadius: "8px",
                    backgroundColor: selectedSource === "rss" ? "rgba(100, 255, 100, 0.1)" : "transparent",
                    border: selectedSource === "rss" ? "1px solid rgba(100, 255, 100, 0.2)" : "none",
                    color: selectedSource === "rss" ? "var(--accent)" : "var(--foreground-muted)",
                    fontSize: "14px",
                    cursor: "pointer",
                    transition: "all 0.2s",
                  }}
              ...
              {selectedSource === "raindrop" && selectedCollection ? (
                <button
              ...
              <button
                onClick={() => {
                  if (selectedSource === "raindrop") {
                    fetchRaindropItems(selectedCollection || undefined);
                  } else {
                    fetchRssItems();
                  }
                }}
          ...
          {selectedSource === "raindrop" && !isConnected ? (
            ...
            >
              ...
              ) : (selectedSource === "raindrop" ? raindropItems.length === 0 : rssArticles.length === 0) ? (
                <div style={{ padding: "40px", textAlign: "center" }}>
                  <p style={{ color: "var(--foreground-muted)", fontSize: "16px" }}>
                    {selectedSource === "raindrop" ? "No items in your reading list" : "No RSS articles available"}
                  </p>
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: "16px" }}>
                  {/* Sidebar - only for Raindrop */}
                  {selectedSource === "raindrop" && (
                    <div
                      style={{
